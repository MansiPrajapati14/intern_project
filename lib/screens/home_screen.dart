...
import 'package:intership/screens/details.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  List<StudenDataModel> studentData = [];
  var jsonData =[
  ...
  List<StudenDataModel> examlist = [];
  List<StudenDataModel> quizlist = [];
  List<StudenDataModel> homeworklist = [];
  ...
  convertData (){
  studentData = studenDataModelFromJson(jsonEncode(jsonData)); 
  for(int i=0;i<studentData.length;i++){  
    if (studentData[i].scores!.firstWhere((element) => element.type == Type.EXAM).score! >=35){
      examlist.add(studentData [i]);
    }
    if (studentData[i].scores!.firstWhere((element) => element.type == Type.QUIZ).score! >=35){
      quizlist.add(studentData [i]);
    }
    if (studentData[i].scores!.firstWhere((element) => element.type == Type.HOMEWORK).score! >=35){
      homeworklist.add(studentData [i]);
    }
  }
  setState(() {
             ...
             Expanded(
               ...
               child: ListView.builder(
                  itemCount: examlist.length,
                   shrinkWrap: true,
                   itemBuilder: (contects,i){
                    return GestureDetector(
                      onTap: (){
                        Navigator.push(context, MaterialPageRoute(builder: (context)=>DetailScreen(datadetails: examlist [i])));
                      },
                      child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Text(examlist[i].name ?? ""),
                      ),
                    );
                   })
                 ),
                 Container(
                     child: ListView.builder(
                         ...
                         itemBuilder: (contects,i){
                           return GestureDetector(
                             onTap: (){
                               Navigator.push(context, MaterialPageRoute(builder: (context)=>DetailScreen(datadetails :quizlist[i] )));
                             },
                             child: Padding(
                               padding: const EdgeInsets.all(8.0),
                               child: Text(quizlist[i].name ?? ""),
                             ),
                           );
                         })
                 ...
                 Container(
                     ...
                     child: ListView.builder(
                         ...
                         itemBuilder: (contects,i){
                           return GestureDetector(
                             onTap: (){
                               Navigator.push(context, MaterialPageRoute(builder: (context)=>DetailScreen(datadetails: homeworklist[i])));
                             },
                             child: Padding(
                               padding: const EdgeInsets.all(8.0),
                               child: Text(homeworklist[i].name ?? ""),
                             ),
                           );
                         })
                 ),
